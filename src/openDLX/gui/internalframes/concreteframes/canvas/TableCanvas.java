...
public class TableCanvas extends JPanel{
	...
	private int textFieldOffset = 150;
	private int outputX = 50; //100 + tableX + table width
	int offsetSize;
	...
	boolean arrowDirection = false; //false is input, true is output
		...
		public TableCanvas(JTable table,int offsetSize) {
		...
		/**
		 * Highlights a data way on the canvas, set way to -1 to highlight no data way
		 * 
		 * @param way
		 */
		...
		private int getColumnWidth(int colNumber) {
			return table.getColumnModel().getColumn(colNumber).getWidth();
		}
		
		private int getRowX(int rowNumber) {
		...
		private int getColumnX(int colNumber) {
			int r = 0;
